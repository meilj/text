...
\begin{document}
	...
	`	\subsection{save part graph}
			%\newsavebox{\子图名称}  %创建一个保存图形的盒子
			%\savebox{\子图名称}(x-尺寸, y-尺寸)[位置]{子图}  %保存子图
			%\savebox{\子图名称}{\begin{picture}(x-尺寸, y-尺寸) ... \end{picture}} , (x-尺寸, y-尺寸)应预设为（0, 0），因为这样使得可以相对于子图定位
			%\usebox{\子图名称} %显示保存的子图
			\newsavebox{\testcon}
			\savebox{\testcon}(0, 0){%
				\thicklines
		...
		\subsection{Generalized grammar}
			%\begin{picture}(x-尺寸, y-尺寸)(x-偏移, y-偏移) 图形命令 \end{picture}
			%(x-偏移, y-偏移)定义了左下角的坐标
		...
		\subsection{Extended package}
			...
			%把方格的左下角放在(x,y)处，它有lx单位宽，ly单位高，每隔给定的[数]单位就划一条格线，而每个第5条格线要粗一些，并加上标记，默认[数]为10
